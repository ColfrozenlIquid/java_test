...
class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (TreeNode t : tree) {
            ...
            System.out.println("Size: " + t.size + " bytes");
            if (t.file_type.contains("Image")){
                System.out.println("Width: " + t.width);
                System.out.println("Height: " + t.height);
                System.out.println("Compression: " + t.compression);
            }
            if (t.file_type.contains("Directory")){
                System.out.println("Compression: " + t.compression);
            }
            System.out.println();
        }
    ...
    public static void recurse_Directory(File currFile, TreeNode curr_Node, ArrayList<TreeNode> tree, HashMap<String, String> supported_filetypes) throws IOException{
            ...
            long avg_compression = 0;
            int count = 0;
            for (TreeNode c : curr_Node.children){
                avg_compression += c.compression;
                count += 1;
            }
            avg_compression = avg_compression/count;
            curr_Node.set_Compression(avg_compression);
        }
    ...
    static void setImageCompression(TreeNode child_node){
        ...
        long compression = size / (child_node.height * child_node.width * child_node.bit_depth);
